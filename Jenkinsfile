...
// which are part of the zeroone repo will allow modification to the Jenkinsfile.
...
for(int i = 0; i < targets.size(); i++) {
  ...
  tasks["${target}"] = {
    node {
      ...
      withEnv(env) {
        def builderImageName="zeroone-builder-${target}"
        ...
        builderImage.inside("-t") {
          ...
          sh "cp -ra ${pwd}/. /zeroone-src/"

          // restore cache
          def hasCache = false
          try {
            copyArtifacts(projectName: "zeroonepay-zeroone/${BRANCH_NAME}", optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz")
          } catch (Exception e) {
          }
          if (fileExists("ci-cache-${target}.tar.gz")) {
            hasCache = true
            echo "Using cache from zeroonepay-zeroone/${BRANCH_NAME}"
          } else {
            try {
              copyArtifacts(projectName: 'zeroonepay-zeroone/develop', optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz");
            } catch (Exception e) {
            }
            if (fileExists("ci-cache-${target}.tar.gz")) {
              hasCache = true
              echo "Using cache from zeroonepay-zeroone/develop"
            }
          }

          if (hasCache) {
            sh "cd /zeroone-src && tar xzf ${pwd}/ci-cache-${target}.tar.gz"
          } else {
            sh "mkdir -p /zeroone-src/ci-cache-${target}"
          }

          stage("${target}/depends") {
            sh 'cd /zeroone-src && ./ci/build_depends.sh'
          }
          stage("${target}/build") {
            sh 'cd /zeroone-src && ./ci/build_src.sh'
          }
          stage("${target}/test") {
            sh 'cd /zeroone-src && ./ci/test_unittests.sh'
          }
          stage("${target}/test") {
            sh 'cd /zeroone-src && ./ci/test_integrationtests.sh'
          }

          // archive cache and copy it into the jenkins workspace
          sh "cd /zeroone-src && tar czfv ci-cache-${target}.tar.gz ci-cache-${target} && cp ci-cache-${target}.tar.gz ${pwd}/"
        }
